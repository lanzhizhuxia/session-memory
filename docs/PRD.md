...
> 从 AI 编码助手的对话历史中，持续提炼个人工作记忆——决策脉络、工作习惯、技术偏好、未竟之事。输出为 8 个 markdown 文件，构成**开发者的数据分身**：任何 AI 工具读取后即可理解你。

## 0.5 定位

session-memory 是一个**独立项目**。它通过 **Source Adapter 架构**从多个 AI 编码助手（OpenCode、Claude Code 等）的对话历史中提取价值，输出为标准 markdown 文件。

**这 8 个 markdown 文件本身就是产品**——一个持续生长的个人知识体。不是报告，不是日志，而是你的数据分身。
...
| 你的其他 AI 工具 | 读取注入知识库上下文 / system prompt | 任何 AI 工具直接理解你 |
| 你自己 | 直接打开 review | 周五回顾、季度总结、新人 onboarding |

**与其他 AI 工具的关系**：平级。session-memory 产出数据，它们消费数据。
...
| 场景 | 触发 | 消费文件 | 效果 |
|---|---|---|---|
| **AI 冷启动消除** | 新开 session 开发某项目 | 工作画像.md + 技术偏好.md | AI 自动理解你的偏好，不再问"你想用什么 ORM" |
| **决策追溯** | "上次为什么不用 Postgres？" | 决策日志.md | 直接定位到决策记录和 session 来源 |
| **新项目冷启动** | 启动一个新项目 | 技术偏好.md | 技术选型不用再解释一遍 |
| **周五回顾** | 每周五下午 | 未完成线索.md | 清理过期 todo，不遗忘承诺 |
| **跨项目学习** | B 项目遇到类似问题 | 反复痛点.md | A 项目踩过的坑直接复用解法 |
| **季度汇报** | 需要回顾做了什么 | 项目时间线.md | 每个项目的时间线一目了然 |
...
```
OpenCode project (worktree: /Users/you/my-project)  ──┐
                                                       ├──→  逻辑项目 "my-project"
Claude Code project (path: /Users/you/my-project)   ──┘     ├── timeline: 按时间混排，标注来源
                                                          ├── todos: 合并去重
...
| 消息 | ~170K+ |
| 消息片段（part） | ~500K+ |
| Todo | ~2K+（含 completed / pending / in_progress / cancelled） |
...
```json
{
  "auto_detected_noise_projects": [
    { "project": "auto-bot", "sources": ["opencode"] }
  ],
  "rules_triggered": {
    "auto-bot": ["uniform_hour_distribution", "low_user_participation", "dominant_session_share"]
  },
...
**边界划分**：`工作画像.md` 讲"你怎么和 AI 协作"；`技术偏好.md` 讲"你用什么技术栈"。不重叠。
...
- 每个项目从创建到现在的关键里程碑
- 功能演进顺序（先做了什么、后加了什么）
- 项目间的关系（project-A 的某模块 → project-B 继承）
...
**输出**：`项目时间线.md`，按项目 → 日期组织

```markdown
## my-project 项目时间线

### 2026-03-17（Day 1）
- [OC] 从旧项目提取核心模块 (+320/-0, 5 files)
- [OC] 建立知识库框架 (+180/-0, 3 files)
- [CC] 首次 workspace 创建流程 (+95/-0, 2 files)

### 2026-03-18（Day 2）
- [OC] 初始化引导流程（onboarding）(+450/-20, 8 files)
- [CC] MCP Server 集成 (+200/-30, 4 files)
- [OC] AI 建议功能 (+150/-10, 3 files)
- [OC] GitHub 上传
```
...
- [ ] 第三方集成（Phase 2）— *5d ago, from "plan phase 2 features" [OC]*
- [~] 知识库双向同步 — *1d ago, from "knowledge base sync" [CC]*

### another-project（12 项）
...
### MCP 连接不稳定
- **出现频率**: 4 次（project-A ×2, project-B ×2）
- **典型症状**: SSE 连接断开后不自动重连
- **解决模式**: 添加重试 + 指数退避
...
- 工作时段热力图（**原始数据**，与 工作画像.md 的工作节奏互补：此处是每小时 session 计数，工作画像 是 AI 归纳的自然语言洞察）
...
Layer 3: DEEP（AI 批量摘要）
  ...
  │  AI 批量摘要 → 提取决策、偏好、痛点
  │  输出：决策日志.md, 反复痛点.md, 工作画像.md, 本周重点.md
```
...
|---|---|---|---|---|
| **P0** | `项目时间线.md` | 每个项目的 session 标题时间线 | Layer 1 | 追加型 |
| **P0** | `未完成线索.md` | 跨项目未完成 todo 汇总 | Layer 1 | 聚合型 |
| **P1** | `工作模式.md` | 用户首条消息聚类，高频任务类型 | Layer 2 | 聚合型 |
| **P1** | `技术偏好.md` | 跨项目技术关键词提取 | Layer 2 | 聚合型 |
| **P2** | `决策日志.md` | AI 摘要的决策日志 | Layer 3 | 追加型 |
| **P2** | `反复痛点.md` | AI 提取的反复痛点 | Layer 3 | 追加型 |
| **P2** | `工作画像.md` | AI 综合的个人工作画像（核心画像 + 交互风格 + 技术审美） | Layer 3 | 聚合型 |
| **P2** | `本周重点.md` | 滚动窗口：进行中 / 已完成 / 关键决策 | Canonical Views | 聚合型 |
...
| `## 交互风格` | 固定 | 二级标题 |
| `## 语言偏好` | 固定 | 二级标题 |
| `## 工作节奏` | 固定 | 二级标题，**AI 归纳的洞察**：基于 工作模式.md 的原始数据，AI 总结出自然语言描述（如"你是夜猫子，10 点前几乎不开工"） |
| `## 技术审美` | 固定 | 二级标题 |
...
| 文件类型 | 策略 | 说明 |
|---|---|---|
| **追加型** | Append + 去重 | `项目时间线.md`, `决策日志.md`, `反复痛点.md` — 新内容追加到对应 section |
| **聚合型** | 每次全量重建 | `工作画像.md`, `技术偏好.md`, `工作模式.md`, `未完成线索.md`, `本周重点.md` — 需要跨全量数据统计或反映最新状态，增量追加会导致结论偏差或 stale |
...
| `项目时间线.md` | adapter 查询 → 跨源合并 → 按项目/日期分组 → 标注来源 → markdown 渲染 | 小 |
| `未完成线索.md` | adapter 查询 → 跨源合并 → 按项目聚合 → markdown 渲染 | 小 |
...
| 交付物 | 实现 | 工作量 |
|---|---|---|
| `工作模式.md` | adapter.getMessages() → 首条消息 → 关键词分类 → 频次统计 | 中 |
| `技术偏好.md` | 技术关键词匹配 → 跨项目 + 跨源聚合 | 中 |
...
| 交付物 | 实现 | 工作量 |
|---|---|---|
| `决策日志.md` | 高价值 session 筛选 → AI 摘要 → 聚合 | 大 |
| `反复痛点.md` | 同上，侧重问题提取 | 大 |
| `工作画像.md` | 汇总 P0+P1+P2 所有发现 | 中 |
| `本周重点.md` | Canonical Views 编译滚动窗口 | 中 |
...
|---|---|---|
| v0.1 | 2026-03-19 | 初稿 |
| v0.2 | 2026-03-19 | 重定位为"数据分身"；独立项目；通用噪音过滤替代硬编码项目名；补使用场景、消费约定、merge 策略、成本估算；拆分 AI prompt 为单维度；优化高价值 session 筛选；明确工作画像与技术偏好边界 |
